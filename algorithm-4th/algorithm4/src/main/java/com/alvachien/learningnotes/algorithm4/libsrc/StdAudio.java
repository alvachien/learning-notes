 ...
 * Media Framework</a>
 * for reading, writing, and playing audio files. You should be able to read or
 * play files
 * in WAVE, AU, AIFF, and MIDI formats and save them to WAVE, AU, and AIFF
 * formats.
 * The file extensions corresponding to WAVE, AU, AIFF, and MIDI files
 * are {@code .wav}, {@code .au}, {@code .aiff}, and {@code .midi},
 * respectively.
 * Some systems support additional audio file formats, but probably not MP3 or
 * M4A.
 * <p>
 ...
 * You can use the following methods to play an audio file in a background
 * thread
...
public final class StdAudio {
    ...
    public static double[] read(String filename) {
        ...
        // normalize AudioInputStream to 44,100 Hz, 16-bit audio, mono, signed PCM,
        // little endian
        ...
        // extract the audio data and convert to a double[] with each sample between -1
        // and +1
        try {
            ...
            while ((count = toAudioInputStream.read(bytes, 0, READ_BUFFER_SIZE)) != -1) {
                ...
                for (int i = 0; i < count / 2; i++) {
                    double sample = ((short) (((bytes[2 * i + 1] & 0xFF) << 8) | (bytes[2 * i] & 0xFF)))
                            / ((double) MAX_16_BIT);
                 ...
                 * double left = ((short) (((bytes[4*i + 1] & 0xFF) << 8) | (bytes[4*i + 0] &
                 * 0xFF))) / ((double) MAX_16_BIT);
                 * double right = ((short) (((bytes[4*i + 3] & 0xFF) << 8) | (bytes[4*i + 2] &
                 * 0xFF))) / ((double) MAX_16_BIT);
    ...
    public static void save(String filename, double[] samples) {
        ...
        try (ByteArrayInputStream bais = new ByteArrayInputStream(data);
            ...
            } else if (filename.endsWith(".aif") || filename.endsWith(".aiff") || filename.endsWith(".AIF")
